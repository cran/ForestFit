...
fitWeibull<-function(mydata, location, method, starts){
if(location=="FALSE"){
if(method!="mlm" & method!="ml" &
   method!="wmle" & method!="rank" &
   method!="greg2" & method!="lmoment" &
...
if(method!="moment" & method!="ml" &
   method!="modifiedmoment1" & method!="modifiedmoment2" & method!="modifiedmoment3"&
   method!="modifiedml1" & method!="modifiedml2" &
   method!="modifiedml3" &  method!="modifiedml4" & method!="wml"&
   method!="tlmoment" & method!="mps")
...
if(method=="mps"){
sort.x<-sort(mydata)
pdf=function(p,x){dweibull(x-p[3],p[1],p[2])}
cdf=function(p,x){pweibull(x-p[3],p[1],p[2])}
	f1<-function(p,x){
	z=NULL
	z=diff(c(0,cdf(p,x),1))
	for (j in 2:n){if((x[j]-x[j-1])==0){z[j]=pdf(p,x[j])}}
	z[z<1e-16]=1e-16
	-sum(log(z))}
out<-suppressWarnings(optim(c(starts[1],starts[2],starts[3]),fn=f1,x=sort.x,method=
"Nelder-Mead")$par)
}

if(method=="wml"){
n<-length(mydata)
f2<-function(p,x){sum(-dweibull(x-p[3],p[1],scale=p[2],log=TRUE))}
out<-suppressWarnings(optim(c(starts[1],starts[2],starts[3]),fn=f2,x=mydata,method=
"Nelder-Mead")$par)
if (n>100){
...
for (r in 1:10){
f3<-function(p,u){(-sum(J2[n]/(n*p[1]))-sum(log(abs(u-p[2]))/n)+sum(log(abs(u-p[2]))*
(abs(u-p[2]))^p[1])/sum((abs(u-p[2]))^p[1]))^2-(sum(1/(n*(abs(u-p[2]))))*
sum((abs(u-p[2]))^p[1])/sum((abs(u-p[2]))^(p[1]-1))-jj)^2}
hat[r+1,]<-suppressWarnings(optim(c(abs(hat[1,1]),hat[1,2]),fn=f3,u=mydata,method="Nelder-Mead")$par)
jj<-splinefun(seq(.1,3,.1),J3[,n])(hat[r+1,1])
}
beta<-(sum((abs(mydata-hat[11,2]))^(hat[11,1]))/(n*J1[n]))^(1/hat[11,1])
alpha<-hat[11,1]
mu<-hat[11,2]
out<-c(alpha,beta,mu)
}
...
if(method=="ml"){
f4<-function(p,x){sum(-dweibull(x-p[3],p[1],p[2],log=TRUE))}
out<-suppressWarnings(optim(c(starts[1],starts[2],starts[3]),fn=f4,x=mydata,method=
"Nelder-Mead")$par)
}
if(method=="tlmoment"){
s.x<-sort(mydata)
...
m21<-12/(n*(n-1)*(n-2)*(n-3))*(sum((n-i3)*(i3-1)*(i3-2)/2*s.x3)-sum((n-i2)*(n-i2-1)/2*
(i2-1)*s.x2))
...
beta<-m21/(6*gamma(1/alpha+1)*(2^(-1/alpha-1)-2*3^(-1/alpha-1)+4^(-1/alpha-1)-3^
(-1/alpha-1)+4^(-1/alpha-1)))
...
(gamma(1+2/u)-(gamma(1+1/u))^2)^1.5-mean((mydata-mean(mydata))^3)/(var(mydata))^1.5}
alpha<-uniroot(f6,lower=0.02,upper=500)$root
beta<-sqrt(var(mydata)/(2*gamma(1+2/alpha)-(gamma(1+1/alpha))^2))
mu<-mean(mydata)-beta*gamma(1+1/alpha)
out<-c(alpha,beta,mu)
}

if(method=="modifiedmoment1"){
h1<-function(u){(gamma(1+2/u)-(gamma(1+1/u))^2)/
(gamma(1+1/u)-(-log(n/(n+1)))^(1/u))^2-var(mydata)/
(mean(mydata)-min(mydata))^2}
if (h1(0.02)*h1(500)>0){
out<-list("alpha"=0,"beta"=0,"mu"=0)
}else{
alpha<-uniroot(h1,lower=.02,upper=500)$root
beta<-sd(mydata)/sqrt(gamma(1+2/alpha)-(gamma(1+1/alpha))^2)
mu<-mean(mydata)-beta*gamma(1+1/alpha)
out<-c(alpha,beta,mu)
}
}

if(method=="modifiedmoment2"){
h2<-function(u){(gamma(1+2/u)-(gamma(1+1/u))^2)/
(gamma(1+1/u)*(1-n^(-1/u)))^2-var(mydata)/
(mean(mydata)-min(mydata))^2}
if (h2(0.02)*h2(500)>0){
out<-list("alpha"=0,"beta"=0,"mu"=0)
}else{
...
if(method=="modifiedmoment3"){
h3<-function(u){(gamma(1+2/u)-(gamma(1+1/u))^2)/(gamma(1+1/u)-(log(2))^(1/u))-
var(mydata)/(mean(mydata)-median(mydata))^2}
if (h3(0.02)*h3(500)>0){
out<-list("alpha"=0,"beta"=0,"mu"=0)
}else{
...
if(method=="modifiedml1"){
h4<-function(p,x){-n*log(p[1])+n*p[1]*log(p[2])-(p[1]-1)*log(-log(.5))-(p[1]-1)*
sum(log(x-p[3]))+sum(((x-p[3])/p[2])^p[1])-log(.5)}
s.x2<-sort(mydata)[-1]
out<-suppressWarnings(optim(c(starts[1],starts[2],starts[3]),fn=h4,x=s.x2,method=
"Nelder-Mead")$par)
}

if(method=="modifiedml2"){
h5<-function(p,x){-n*log(p[1])+n*p[1]*log(p[2])-
(p[1]-1)*sum(log(x-min(x)+p[2]*gamma(1+1/p[1])*n^(-1/p[1])))+
sum(((x-min(x)+p[2]*gamma(1+1/p[1])*n^(-1/p[1]))/p[2])^p[1])}
out<-suppressWarnings(optim(c(starts[1],starts[2],starts[3]),fn=h5,x=mydata,method=
"Nelder-Mead")$par)
}

if(method=="modifiedml3"){
h6<-function(p,x){-n*log(p[1])+n*p[1]*log(p[2])-(p[1]-1)*sum(log(abs(x-mean(x)+p[2]*
gamma(1+1/p[1]))))+
sum((abs((x-mean(x)+p[2]*gamma(1+1/p[1])))/p[2])^p[1])}
out<-suppressWarnings(optim(c(starts[1],starts[2]),fn=h6,x=mydata,method=
"Nelder-Mead")$par)
mu<-mean(mydata)-out[2]*gamma(1+1/out[1])
out<-c(out[1],out[2],mu)
}

if(method=="modifiedml4"){
h7<-function(p,x){-n*log(p[1])+n*p[1]*log(p[2])-(p[1]-1)*sum(log(abs(x-median(x)+p[2]*
(log(2))^(1/p[1]))))+
sum((x-median(x)+p[2]*(log(2))^(1/p[1]))^(p[1]))}
out<-suppressWarnings(optim(c(starts[1],starts[2]),fn=h7,x=mydata,method=
"Nelder-Mead")$par)
mu<-median(mydata)-out[2]*(log(2))^(1/out[1])
out<-c(out[1],out[2],mu)
}
}
...
if(method=="wml"){
if (starts[1]>3){
h8<-function(p,x){sum(-dweibull(x,p[1],scale=p[2],log=TRUE))}
out<-suppressWarnings(optim(c(starts[1],starts[2]),fn=h8,x=mydata,method=
"Nelder-Mead")$par)
}
if (n>=100 & starts[1]<3){
h9<-function(p,x){sum(-dweibull(x,p[1],scale=p[2],log=TRUE))}
out<-suppressWarnings(optim(c(starts[1],starts[2]),fn=h9,x=mydata,method=
"Nelder-Mead")$par)
}
if (n<=100 & starts[1]<3){
J1<-c(
...
h10<-function(p,x){(J2[length(x)]/p[1]+mean(log(x))-sum(log(x)*x^p[1])/sum(x^p[1]))^2}
alpha<-suppressWarnings(optim(starts[[1]],fn=h10,x=mydata,method="Nelder-Mead")$par)
beta<-(sum(mydata^alpha)/(n*J1[n]))^(1/alpha)
out<-c(alpha,beta)
}
}

if(method=="rank"){
d<-cor(mydata,rank(mydata))*sqrt(var(mydata))/(sqrt(3)*mean(mydata))*sqrt((n+1)/(n-1))
alpha<--log(2)/log(1-d)
beta<-(sum(mydata^(alpha))/n)^(1/alpha)
out<-c(alpha,beta)
}

if(method=="reg"){
k<-seq(1,n)
fit<-lm(log(-log(1-(k-.3)/(n+.4)))~log(sort(mydata)))
alpha<-fit[[1]][[2]]
...
if(method=="moment"){
h11<-function(u){(gamma(1+2/u)-(gamma(1+1/u))^2)/(gamma(1+1/u))^2-var(mydata)/
(mean(mydata))^2}
alpha<-uniroot(h11,lower=0.02,upper=500000)$root
beta<-mean(mydata)/gamma(1+1/alpha)
out<-c(alpha,beta)
}
...
y<-sort(mydata)
k<-seq(1,n)
X<-cbind(rep(1,n),log(-log(1-k/(n+1)))+(k*(n-k+1))/((n+1)^2*(n+2))*(log(1-k/(n+1))+1)/
((1-k/(n+1))*log(1-k/(n+1)))^2)
V<-matrix(0,n,n)
for(i in 1:n){for(j in i:n){V[i,j]<-i/(n+1-i)*(log((n+1-i)/(n+1))*log((n+1-j)/(n+1)))^(-1)}}
U<-solve(t(X)%*%solve(V)%*%X)%*%t(X)%*%solve(V)%*%log(y)
beta<-exp(U[1])
...
if(method=="ml"){
h12<-function(p,x){sum(-dweibull(x,shape=p[1],scale=p[2],log=TRUE))}
out<-suppressWarnings(optim(c(starts[1],starts[2]),fn=h12,x=mydata,method="Nelder-Mead")$par)
}
...
if(method=="mlm"){
alpha<-sqrt(pi^2/(6*var(log(mydata))))
out<-c(alpha,exp((mean(log(mydata))+0.5772156)/alpha))
}

if(method=="pm"){
p<-0.31
qu<-quantile(mydata,0.6321206)[[1]]
alpha<-log(-log(1-p))/(log(quantile(mydata,p)[[1]]/qu))
out<-c(alpha,qu)
}
...
for(i in 1:(n-1)){
for(j in (i+1):n){
sa<-sa+(log(min(mydata[i],mydata[j]))-(log(mydata[i])+log(mydata[j]))/2)/log(2)
sb<-sb-0.57721566*log(min(mydata[i],mydata[j]))/log(2)+(log(mydata[i])+log(mydata[j]))/
...
out<-c(alpha,beta)
  ...
  log.likelihood=suppressWarnings(sum(log(round(pdf0(out,mydata), digits=20))))
  von<-u<-c()
  anderson<-c()
  sx<-sort(mydata)
  for(i in 1:n){
  ...
  BIC=-2*log.likelihood + n.p*log(n)
  HQIC=-2*log.likelihood + 2*log(log(n))*n.p
  ks.stat=suppressWarnings(ks.test(mydata, "cdf0",par=out))[[1]][[1]]
  out1=cbind(AIC, CAIC, BIC, HQIC, ks.stat, anderson.stat, von.stat, log.likelihood)
  oux=cbind(out[1],out[2])
  colnames(oux)=c("shape","scale")
  if (location=="TRUE"){
  oux=cbind(out[1],out[2],out[3])
  colnames(oux)=c("shape","scale","location")
  }
  colnames(out1)=c("AIC","CAIC","BIC","HQIC","KS","AD","CVM","log")
  list("estimate"=oux,"measures"=out1)
}
dmixture<-function(x,g,K,param){
 ...
 # if (sum(param[1:K])!= 1){stop ("The weight vector must be added to one.")}
  ...
  if(g=="weibull"){
      ...
      for (i in 1:length(x)){
          pdf<-0
            for (j in 1:K){
                pdf<-pdf+omega[j]*den(x[i],c(alpha[j],beta[j],lambda[j]))
            }
        y[i]<-pdf
      }
return(y)
 ...
 # if (sum(param[1:K])!= 1){stop ("The weight vector must be added to one.")}
  ...
  if(g=="weibull"){
      ...
      for (i in 1:length(x)){
		      cdf<-0
			       for (j in 1:K){
			        	  cdf<-cdf+omega[j]*cum(x[i],c(alpha[j],beta[j],lambda[j]))
  		       }
		      y[i]<-cdf
     }
return(y)
...
#  if (sum(omega)!= 1){stop ("The weight vector must be added to one.")}
   size<-apply(rmultinom(n,1,omega),1,sum)
     y<-rep(NA,n)
     s<-0
  if(g=="birnbaum-saunders"){
...
clust<-kmeans(x,K,50,1,algorithm="Hartigan-Wong")
  ...
  if(initial=="TRUE"){
    ...
    alpha.matrix[1,]<-starts[(K+1):(2*K)]
    beta.matrix[1,]<-starts[(2*K+1):(3*K)]
    for (i in 1:K) z[,i]<-p.matrix[1,i]*denbir(x,c(alpha.matrix[1,i],beta.matrix[1,i]))
  ...
  while ((eps>cri) && (r<N)){
	...
	beta<-beta.matrix[r,]
	pdf0<-dmixture(x,"birnbaum-saunders",K,c(weight,alpha,beta))
	cdf0<-pmixture(sort(x),"birnbaum-saunders",K,c(weight,alpha,beta))
}
if (family=="burr"){
  if(initial=="FALSE"){
        for  (i in 1:K){
               ...
               nn<-length(y)
   		     z1=NULL;
    		   burr.log<-function(par,x){-nn*log(par[1])-nn*log(par[2])-(par[1]-1)*sum(log(x))+(par[2]+1)*sum(log(1+x^par[1]))}
	...
	if(initial=="TRUE"){
	  ...
	  alpha.matrix[1,]<-starts[(K+1):(2*K)]
	  beta.matrix[1,]<-starts[(2*K+1):(3*K)]
	  for (i in 1:K) z[,i]<-p.matrix[1,i]*denburr(x,c(alpha.matrix[1,i],beta.matrix[1,i]))
	  d<-cbind(c(1:n),apply(z, 1, which.max))
	  z<-matrix(0,nrow=n,ncol=K)
	  z[d]<-1
	}
	...
	alpha<-alpha.matrix[r,]
	beta<-beta.matrix[r,]
	pdf0<-dmixture(x,"burr",K,c(weight,alpha,beta))
	cdf0<-pmixture(sort(x),"burr",K,c(weight,alpha,beta))
}
if (family=="chen"){
  if(initial=="FALSE"){
    for  (i in 1:K){
	...
	if(initial=="TRUE"){
	  ...
	  alpha.matrix[1,]<-starts[(K+1):(2*K)]
	  beta.matrix[1,]<-starts[(2*K+1):(3*K)]
	  for (i in 1:K) z[,i]<-p.matrix[1,i]*denchen(x,c(alpha.matrix[1,i],beta.matrix[1,i]))
	  d<-cbind(c(1:n),apply(z, 1, which.max))
	  z<-matrix(0,nrow=n,ncol=K)
	  z[d]<-1
	}
	...
	alpha<-alpha.matrix[r,]
	beta<-beta.matrix[r,]
	pdf0<-dmixture(x,"chen",K,c(weight,alpha,beta))
	cdf0<-pmixture(sort(x),"chen",K,c(weight,alpha,beta))
}
if (family=="f"){
  if(initial=="FALSE"){
    for  (i in 1:K){
	...
	if(initial=="TRUE"){
	  ...
	  alpha.matrix[1,]<-starts[(K+1):(2*K)]
	  beta.matrix[1,]<-starts[(2*K+1):(3*K)]
	  for (i in 1:K) z[,i]<-p.matrix[1,i]*denf(x,c(alpha.matrix[1,i],beta.matrix[1,i]))
	  d<-cbind(c(1:n),apply(z, 1, which.max))
	  z<-matrix(0,nrow=n,ncol=K)
	  z[d]<-1
	}
	...
	alpha<-alpha.matrix[r,]
	beta<-beta.matrix[r,]
	pdf0<-dmixture(x,"f",K,c(weight,alpha,beta))
	cdf0<-pmixture(sort(x),"f",K,c(weight,alpha,beta))
}
if (family=="frechet"){
  if(initial=="FALSE"){
      clust<-kmeans(1/x,K)
	...
	if(initial=="TRUE"){
	  ...
	  alpha.matrix[1,]<-starts[(K+1):(2*K)]
	  beta.matrix[1,]<-starts[(2*K+1):(3*K)]
	  for (i in 1:K) z[,i]<-p.matrix[1,i]*denfre(x,c(alpha.matrix[1,i],beta.matrix[1,i]))
	  d<-cbind(c(1:n),apply(z, 1, which.max))
	  z<-matrix(0,nrow=n,ncol=K)
	  z[d]<-1
	}
	...
	alpha<-alpha.matrix[r,]
	beta<-beta.matrix[r,]
	pdf0<-dmixture(x,"frechet",K,c(weight,alpha,beta))
	cdf0<-pmixture(sort(x),"frechet",K,c(weight,alpha,beta))
}
if (family=="gamma"){
  if(initial=="FALSE"){
    for  (i in 1:K){
	...
	if(initial=="TRUE"){
	  ...
	  alpha.matrix[1,]<-starts[(K+1):(2*K)]
	  beta.matrix[1,]<-starts[(2*K+1):(3*K)]
	  for (i in 1:K) z[,i]<-p.matrix[1,i]*dgamma(x,shape=alpha.matrix[1,i],scale=beta.matrix[1,i])
	  d<-cbind(c(1:n),apply(z, 1, which.max))
	  z<-matrix(0,nrow=n,ncol=K)
	  z[d]<-1
	}
	...
	alpha<-alpha.matrix[r,]
	beta<-beta.matrix[r,]
	pdf0<-dmixture(x,"gamma",K,c(weight,alpha,beta))
	cdf0<-pmixture(sort(x),"gamma",K,c(weight,alpha,beta))
}
if (family=="gompertz"){
  if(initial=="FALSE"){
    for  (i in 1:K){
           ...
           nn<-length(y)
  		     qp3<-y[floor(.75*nn)]
		       m.mydata<-y[floor(.5*nn)]
	         a.hat<-log(log(1-.75)/log(1-.5))/(qp3-m.mydata)
    		   b.hat<--a.hat*log(0.5)/(exp(a.hat*m.mydata)-1)
    		   z1=NULL;
	...
	if(initial=="TRUE"){
	  ...
	  alpha.matrix[1,]<-starts[(K+1):(2*K)]
	  beta.matrix[1,]<-starts[(2*K+1):(3*K)]
	  for (i in 1:K) z[,i]<-p.matrix[1,i]*dengom(x,c(alpha.matrix[1,i],beta.matrix[1,i]))
	  d<-cbind(c(1:n),apply(z, 1, which.max))
	  z<-matrix(0,nrow=n,ncol=K)
	  z[d]<-1
	}
	...
	alpha<-alpha.matrix[r,]
	beta<-beta.matrix[r,]
	pdf0<-dmixture(x,"gompertz",K,c(weight,alpha,beta))
	cdf0<-pmixture(sort(x),"gompertz",K,c(weight,alpha,beta))
}
if (family=="log-logistic"){
  if(initial=="FALSE"){
	...
	if(initial=="TRUE"){
	  ...
	  alpha.matrix[1,]<-starts[(K+1):(2*K)]
	  beta.matrix[1,]<-starts[(2*K+1):(3*K)]
	  for (i in 1:K) z[,i]<-p.matrix[1,i]*denlog(x,c(alpha.matrix[1,i],beta.matrix[1,i]))
	  d<-cbind(c(1:n),apply(z, 1, which.max))
	  z<-matrix(0,nrow=n,ncol=K)
	  z[d]<-1
	}
	...
	alpha<-alpha.matrix[r,]
	beta<-beta.matrix[r,]
	pdf0<-dmixture(x,"log-logistic",K,c(weight,alpha,beta))
	cdf0<-pmixture(sort(x),"log-logistic",K,c(weight,alpha,beta))
}
if (family=="log-normal"){
  if(initial=="FALSE"){
    for  (i in 1:K){
               ...
               y<-sort(x[clust$cluster==i])
               nn<-length(y)
               median.mydata<-median(y)
               alpha[i]<-log(median.mydata)
               beta[i]<-sqrt(2*abs(log(mean(y)/median.mydata)))
        }
	alpha.matrix[1,]<-alpha
	beta.matrix[1,]<-beta
	p.matrix[1,]<-portion
	...
	if(initial=="TRUE"){
	  ...
	  alpha.matrix[1,]<-starts[(K+1):(2*K)]
	  beta.matrix[1,]<-starts[(2*K+1):(3*K)]
	  for (i in 1:K) z[,i]<-p.matrix[1,i]*dlnorm(x,meanlog=alpha.matrix[1,i],sdlog=beta.matrix[1,i])
	  d<-cbind(c(1:n),apply(z, 1, which.max))
	  z<-matrix(0,nrow=n,ncol=K)
	  z[d]<-1
	}
	...
	alpha<-alpha.matrix[r,]
	beta<-beta.matrix[r,]
	pdf0<-dmixture(x,"log-normal",K,c(weight,alpha,beta))
	cdf0<-pmixture(sort(x),"log-normal",K,c(weight,alpha,beta))
}
if (family=="lomax"){
  if(initial=="FALSE"){
    for  (i in 1:K){
	...
	if(initial=="TRUE"){
	  ...
	  alpha.matrix[1,]<-starts[(K+1):(2*K)]
	  beta.matrix[1,]<-starts[(2*K+1):(3*K)]
	  for (i in 1:K) z[,i]<-p.matrix[1,i]*denlom(x,c(alpha.matrix[1,i],beta.matrix[1,i]))
	  d<-cbind(c(1:n),apply(z, 1, which.max))
	  z<-matrix(0,nrow=n,ncol=K)
	  z[d]<-1
	}
	...
	alpha<-alpha.matrix[r,]
	beta<-beta.matrix[r,]
	pdf0<-dmixture(x,"lomax",K,c(weight,alpha,beta))
	cdf0<-pmixture(sort(x),"lomax",K,c(weight,alpha,beta))
}
if (family=="weibull"){
  if(initial=="FALSE"){
    for  (i in 1:K){
	...
	if(initial=="TRUE"){
	  ...
	  alpha.matrix[1,]<-starts[(K+1):(2*K)]
	  beta.matrix[1,]<-starts[(2*K+1):(3*K)]
	  for (i in 1:K) z[,i]<-p.matrix[1,i]*dweibull(x,shape=alpha.matrix[1,i],scale=beta.matrix[1,i])
	  d<-cbind(c(1:n),apply(z, 1, which.max))
	  z<-matrix(0,nrow=n,ncol=K)
	  z[d]<-1
	}
	...
	while ((eps>cri) && (r<N)){
					  for (j in 1:K){
						...
						alpha.matrix[r+1,j]<-out[1]
						beta.matrix[r+1,j]<-out[2]
						s.pdf[,j]<-p.matrix[r,j]*dweibull(x,shape=alpha.matrix[r+1,j],scale=beta.matrix[r+1,j])
					  }
							    for (i in 1:n){
									      tau.matrix[i,]<-s.pdf[i,]/sum(s.pdf[i,])
										maxim<-tau.matrix[i,1]
	...
	alpha<-alpha.matrix[r,]
	beta<-beta.matrix[r,]
	pdf0<-dmixture(x,"weibull",K,c(weight,alpha,beta))
...
fitbayesJSB<-function(y,n.burn=8000,n.simul=10000){
  lambda.sampler<-function(gamma,delta,xi,x,nrep=1000){
    ...
    target.ratio<-function(lam.new,lam.old,gamma,delta,xi,x){
      n<-length(x)
      n*log(lam.new/lam.old)+(1-gamma*delta)*sum(log((lam.old+xi-x)/(lam.new+xi-x)))-delta^2/2*sum((log((x-xi)/(lam.new+xi-x)))^2-(log((x-xi)/(lam.old+xi-x)))^2)
    }
    lambda<-c()
    lambda[1]<-Max-xi+1/n
    for (i in 2:nrep){
  ...
  xi.sampler<-function(gamma,delta,lambda,x,nrep=1000){
    Min<-min(x);Max<-max(x)
    n<-length(x)
    target.ratio<-function(xi.n,xi.o,gam,del,lam,x){
      sum(log((x-xi.o)/(x-xi.n)))+sum(log((lam+xi.o-x)/(lam+xi.n-x)))-1/2*sum((gam+del*log((x-xi.n)/(lam+xi.n-x)))^2)+
      1/2*sum((gam+del*log((x-xi.o)/(lam+xi.o-x)))^2)
    }
    xi<-c()
  ...
  stat.JSB<-function(y,hat){
    ...
    del*lam/(sqrt(2*pi)*(x-xi)*(lam+xi-x))*
  ...
  n<-length(y)
  delta.hat[1]<-1
  lambda.hat[1]<-max(y)-min(y)+2/n
  xi.hat[1]<-min(y)-1/n
  gamma.hat[1]<-log(1/median((y-xi.hat[1])/lambda.hat[1])-1);
  for (r in 2:n.simul){
    dd<-log((y-xi.hat[r-1])/(lambda.hat[r-1]+xi.hat[r-1]-y))
    ...
    delta.hat[r]<-ars(1,f,fprima,x=c(0.001,5,33),m=3,lb=TRUE,xlb=0,gamma=gamma.hat[r],lambda=lambda.hat[r-1],xi=xi.hat[r-1],y=y)
    lambda.hat[r]<-lambda.sampler(gamma.hat[r],delta.hat[r],xi.hat[r-1],y,1000)
    xi.hat[r]<-xi.sampler(gamma.hat[r],delta.hat[r],lambda.hat[r],y,1000)
  }
  ...
  out3<-stat.JSB(y,c(deltahat,gammahat,lambdahat,xihat))
...
fitbayesWeibull<-function(y,n.burn=8000,n.simul=10000){
  ...
  if(n.burn>n.simul){stop ("n.burn must be less than n.simul")}
  h<-function(u){(gamma(1+2/u)-(gamma(1+1/u))^2)/(gamma(1+1/u))^2-var(y)/(mean(y))^2}
  alpha0<-uniroot(h,lower=0.02,upper=500000)$root
  beta0<-mean(y)/gamma(1+1/alpha0)
  ...
  alpha.hat[1]<-alpha0
  beta.hat[1]<-beta0
  mu.hat[1]<-min(y)-1/length(y)
  for (r in 1:n.simul){
    z<-rgamma(1,length(y),1)
    beta.hat[r+1]<-(sum((y-mu.hat[r])^alpha.hat[r])/z)^(1/alpha.hat[r])
    alpha.hat[r+1]<-ars(1,f.log,f.logprim,x=c(.05,.2,2,4,40),ns=200,m=5,emax=64,lb=TRUE,xlb=0,b=beta.hat[r+1],mu=mu.hat[r],y=y)
    mu.hat[r+1]<-mu.sampler(alpha.hat[r+1],beta.hat[r+1],y)
  }
  ...
  out3<-suppressWarnings(stat.W(y,c(alphahat,betahat,muhat)))
  out2<-cbind(out3[1],out3[2],out3[3],out3[4])
  colnames(out1)=c("alpha","beta","mu")
  colnames(out2)=c("AD","CVM","KS","log-likelihood")
  list("estimate"=out1,"measures"=out2)
